...
private _fnc_getIcon =
{
    ...
    // If still empty try to get it from vehicle icon class
...
private _fnc_createModuleIcon =
{
    ...
    _ctrlModule ctrlSetText ([_cfgModule] call _fnc_getIcon);
...
private _angleStep = 360 / (count _syncClasses);
{
    ...
    [_forEachIndex * _angleStep, R1, _centerX, _centerY] call _fnc_calculatePosition params ["_xPos", "_yPos"];
...

    {
        ...
        [_indexSubItem * _angleStep, R2, _centerX, _centerY] call _fnc_calculatePosition params ["_xPos", "_yPos"];
        [_cfgModule, _xPos, _yPos, _onMouseEnter, _onMouseExit] call _fnc_createModuleIcon;
        [_displayModuleInformation, _centerX, _centerY, _xPos, _yPos] call _fnc_drawLine;
    } forEach _subSync;
